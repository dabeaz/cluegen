...
def cluegen(func):
    ...
    return type(
        f"ClueGen_{func.__name__}", (), dict(__get__=__get__, __set_name__=__set_name__)
    )()
...
class Datum(DatumBase):
    ...
    def __init__(cls):
        ...
        args = ", ".join(
            ...
            if hasattr(cls, name)
            and not isinstance(getattr(cls, name), types.MemberDescriptorType)
            else name
            for name in clues
        )
    ...
    def __hash__(cls):
        clues = all_clues(cls)
        if clues:
            self_tuple = "".join(["(", ",".join(f"self.{name}" for name in clues), ")"])
        else:
            self_tuple = "()"
